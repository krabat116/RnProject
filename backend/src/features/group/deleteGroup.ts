...
const deleteGroupRoute = createRoute({
  method: 'delete',
  path: '/group/{groupId}',
  request: { params: deleteGroupParams },
  responses: {
    200: {
    ...
    400: {
      description: 'missing or invalid groupId.',
      content: { 'application/json': { schema: Response } },
    },
    500: {
...
deleteGroup.openapi(deleteGroupRoute, async (c) => {
  try {
    ...
    const groupId = c.req.param('groupId')
    if (!groupId) {
      return c.json({ type: 'ERROR', message: 'Missing groupId.' }, 400)
    }
    ...
    // 로그를 꼭 남겨야 실제 원인을 바로 잡습니다.
    console.error('deleteGroup failed:', error)

    return c.json({ type: 'ERROR', message: 'Failed to delete group.' }, 500)
  }
})
